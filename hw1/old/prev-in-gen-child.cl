  ...
  (cond
   ...
   (T (dfs
        (append
          ...
          (rest open-list))
        (cons (first open-list) closed-list) goals
        moves (+ 1 die)))))
...
(defun generate-children (node moves)
  (cond
   ...
   (T (cons
        (list
         (funcall (first moves) (first node))
         (cond
          ((mymember (first node) (second node)) (second node)) ; don't add current state to prev state if it's already in there
          (T (cons (first node) (second node))) ; add current state to prev state since not in there yet.
          ))
...
;    ; only check the current state while filtering, instead of entire path.
;    ((mymember (first (first children-list)) check-list)
...
; (append (generate-children '((3 3 W)) *moves*) (rest (LIST (list *start* '()))))
